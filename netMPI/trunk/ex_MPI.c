...
int usage(){
	...
	printf("-b,\t --min_coef \t\t Bit left switch defining the size of the smallest test (in bytes) (DEFAULT: 0)\n");
	printf("-c,\t --max_coef \t\t Bit left switch defining the size of the biggest test (in bytes) (DEFAULT: 22)\n");
	printf("-s,\t --skip	\t\t Effectively skip the s first tests (DEFAULT: 3).\n");
	...
	printf("\n\n");
	printf("Example: ex_MPI --isr -m 2 -s 2 -n 5 -b 21 -r 4 -p example.log");
...
int main(int argc, char **argv)
{
	...
	static struct option long_options[] = {
		...
		{"max_coef", required_argument, 0, 'c'},
		{"min_coef", required_argument, 0, 'b'},
	...
	while(1){
		...
		g = getopt_long (argc, argv, "hm:b:c:s:n:r:p:", long_options, &option_index); 
		...
		switch(g){
			...
			case 'b':
				min_coefs = (int)atoi(optarg); break;
	...
	for(i=0;i<nb_methods;i++){
		arr_results[i] = (double **)malloc(coefs*sizeof(double *));
		for(c=0;c<coefs;c++)
	...
	if(fl_sr==1){
		e1-=MPI_Wtime();
		for(c = 0; c < coefs; c++){
			arr_results[0][c] = net_MPI_bdw_sr(master_rank, nbtasks, myrank, 1<<(c+min_coefs), nb_tests, skip, latency);
		}
		e1+=MPI_Wtime();
	}
	...
	if(fl_bsr==1){
		e2-=MPI_Wtime();
		for(c = 0; c < coefs; c++)
			arr_results[1][c] = net_MPI_bdw_bsr(master_rank, nbtasks, myrank, 1<<(c+min_coefs), nb_tests, skip, latency);
		e2+=MPI_Wtime();
	}
	...
	if(fl_isr==1){
		e3-=MPI_Wtime();
		for(c = 0; c < coefs; c++)
			arr_results[2][c] = net_MPI_bdw_isr(master_rank, nbtasks, myrank, 1<<(c+min_coefs), nb_tests, skip, isr_requests, latency);
		e3+=MPI_Wtime();
	}
	...
	if(fl_bisr==1){
		e4-=MPI_Wtime();
		for(c = 0; c < coefs; c++)
			arr_results[3][c] = net_MPI_bdw_bisr(master_rank, nbtasks, myrank, 1<<(c+min_coefs), nb_tests, skip, isr_requests, latency);
		e4+=MPI_Wtime();
	}
	...
	if(myrank==master_rank){
		...
		if(fl_lat==1){
			...
			displ_txtp(stream, &latency, 0, 1, nbtasks, myrank);
		}
		...
		if(fl_sr==1){
			...
			displ_txtp(stream, arr_results[0], min_coefs, coefs, nbtasks, myrank);		
		}
		...
		if(fl_bsr==1){
			...
			fprintf(stream, "# Bandwidth between %d and the world, in MBits/s\n", master_rank);
			displ_txtp(stream, arr_results[1], min_coefs, coefs, nbtasks, myrank);		
		}
		...
		if(fl_isr==1){
			...
			fprintf(stream, "# Bandwidth between %d and the world, in MBits/s\n", master_rank);
			displ_txtp(stream, arr_results[2], min_coefs, coefs, nbtasks, myrank);		
		}
		...
		if(fl_bisr==1){
			...
			fprintf(stream, "# Bandwidth between %d and the world, in MBits/s\n", master_rank);
			displ_txtp(stream, arr_results[3], min_coefs, coefs, nbtasks, myrank);		
		}
